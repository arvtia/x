...
const API_BASE = import.meta.VITE_BASE_URL;
...
const PasswordResetFrom = () => {
   ...
   const handleResetPassword = async (e) =>{
      ...
      try 
      {
         const response = await axios.post ( `${import.meta.env.VITE_BASE_URL}/api/auth/reset-password`,
            {password},
                     ...
                     <form onSubmit={handleResetPassword}>
                           <div className="backdrop-blur-sm grid grid-cols-1 outline-5 outline-dashed outline-white  rounded-lg">
                              ...
                              <div className="p-4 relative">
                                 ...
                                 <input onChange={(e)=> setPassword(e.target.value)} type="password" name="password" id="password" className="relative py-3 px-2 bg-neutral-100 w-full outline-indigo-400 rounded-md focus:outline-2 mt-1 focus:outline-dashed focus:outline-offset-1 shadow-[01px_2px_#ddd]"/>
                                 {/* <i className="bi bi-eye absolute top-1/2 right-8 z-40"></i> */}
                              </div>
                              <div className=" w-full p-4 relative">
                                 <label htmlFor="submit" className="text-sm text-neutral-300 text-wrap w-full">By clicking Submit you are agreeing to reseting you password</label>
